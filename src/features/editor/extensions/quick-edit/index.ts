...
export const quickEditState = StateField.define<boolean>({
  ...
  update(value, transaction) {
    // Priority 1: If an explicit toggle effect is dispatched, use it immediately
    ...
    // Priority 2: If the user clears their selection, auto-close the modal
...
const quickEditTooltipField = StateField.define<readonly Tooltip[]>({
  ...
  update(tooltips, transaction) {
    // Priority 1: Check if visibility state was explicitly toggled via effect first
    for (const effect of transaction.effects) {
      if (effect.is(showQuickEditEffect)) {
    ...
    // Priority 2: Fallback to document structural or selection positioning changes
...
const quickEditKeymap = keymap.of([
  {
    key: "Mod-k", // inspired from cursor
    run: (view) => {
  ...
  {
    key: "Mod-i", // inspired from antigravity
    run: (view) => {
      const selection = view.state.selection.main;
      if (selection.empty) return false;
